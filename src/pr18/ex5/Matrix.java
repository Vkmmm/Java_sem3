...
public class Matrix< N extends Number> {
    ...
    public void sum(Matrix matrix0, Matrix matrixr) {
        for (int i = 0; i < nMatrix.length; i++) {
            for (int j = 0; j < nMatrix[i].length; j++) {
                matrixr.nMatrix[i][j] = Calc.sum(matrix0.nMatrix[i][j], nMatrix[i][j]);
            }
        }
    }
    public void subtraction(Matrix matrix0, Matrix matrixr) {
        for (int i = 0; i < nMatrix.length; i++) {
            for (int j = 0; j < nMatrix[i].length; j++) {
                matrixr.nMatrix[i][j] = Calc.subtraction(nMatrix[i][j], matrix0.nMatrix[i][j]);
            }
        }
    }
    public N det() {
        return Calc.subtraction(Calc.multiply(nMatrix[0][0],nMatrix[1][1]),
                Calc.multiply(nMatrix[0][1],nMatrix[1][0]));
    }
